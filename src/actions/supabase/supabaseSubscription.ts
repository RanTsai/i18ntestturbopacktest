...
//base function to getSubscription from Supabase
...
//base function to insert new purchase to user purchase history
...
//-----------------------------------------------------Logic Functions---------------------------------------------------
// Logic function to save purchase to Supabase
export async function savePurchaseToSupabase(product: IProduct, user_balance: number) {
    try {
        ...
        if (error) {
            ...
            const { success } = await insertNewPurchaseToUserCreditHistory(
