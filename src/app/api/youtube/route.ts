  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 403) {
        ...
        const fallback = await GetFallBackVideoThumbnails("default");
        console.log("fallback", fallback, "fallback data", fallback.data);
        if (fallback.success && fallback.data && fallback.data.videos) {
          const videos = fallback.data.videos.map((item: any, idx: number) => ({
            id: item.id || `vid-${idx}`,
            title: item.title,
