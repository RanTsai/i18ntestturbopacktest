...
export type IChatMessage = {
  ...
  user_feedback: boolean | null; //True for Like, false for dislike, null for no feedback
...
type AIThreadStore = {
  ...
  patchMessage: (tid: string, messageIdOrIndex: { id?: string | number; index?: number }, patch: Partial<IChatMessage>) => void;
  setMessageFeedbackLocal: (tid: string, messageIndex: number, feedback: boolean | null) => void;
...
export const useAIChatStore = create<AIThreadStore>()(
  persist(
    (set, get) => ({
      ...
      patchMessage: (tid, key, patch) => set((s) => {
        const bucket = s.messagesByThread[tid];
        if (!bucket?.items?.length) return {};
        const items = bucket.items.map(m => {
          const hit = key.id != null
            ? String(m.chat_message_id) === String(key.id)
            : key.index != null
              ? m.message_index === key.index
              : false;
          return hit ? { ...m, ...patch } : m;
        });
        return { messagesByThread: { ...s.messagesByThread, [tid]: { ...bucket, items } } };
      }),
      setMessageFeedbackLocal: (tid, messageIndex, feedback) =>
        get().patchMessage(tid, { index: messageIndex }, { user_feedback: feedback }),
