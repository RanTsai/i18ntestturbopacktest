...
import { useSearchParams } from "next/navigation";              // ⬅️ 新增
...
import { CollapsibleCard } from "../../chattest/collapsiblecard";
...
export default function ClientPage() {
    ...
    const { loadChatThreads, userWorkPublicId, versionNumber, selectThread, setMessagesLoading } =
        useAiChatThreadViewModel();

  useEffect(() => {
    let cancelled = false;
    (async () => {
      await loadChatThreads(userWorkPublicId ?? null, versionNumber ?? null);
      if (cancelled) return;

      const t = searchParams.get("t");
      if (t) {
        // 🔑 提前標記 loading，避免畫面一片空白
        setMessagesLoading(t, true);
        selectThread(t);
      }
    })();
  ...
  }, [searchParams, userWorkPublicId, versionNumber]);
